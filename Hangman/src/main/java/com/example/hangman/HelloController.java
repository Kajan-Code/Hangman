...
public class HelloController {
    ...
    protected void InputButtonClick () {
        ...
        if (guessedWord.equalsIgnoreCase(wort)) {
            guessField.setText("Richtig du hast das Wort! erraten");
            wortField.setText(Eingabe.getText());

        }

        if (guessField.getLength() == 1 && wort.contains(guessedWord)) { // Wenn die Länge des ErratenenFeldes 1 ist und im Wort ist dann wird es ausgeführt
            char guessedChar = guessedWord.charAt(0); // Nimmt den Buchstaben an der Stelle 0, im Feld: guessField
            StringBuilder updatedSolution = new StringBuilder(solutionText); //
            ...
            if (!(wortField.getText().contains("_"))) {
                guessField.setText("Du hast das Wort erraten!");
                Text.setText("");
            }
        }
        ...
        else if (guessField.getLength() == 1 && !(wort.contains(guessedWord.toLowerCase()))) {
        if (fehler <= 9) {
